(function () {
  ...
  function classify(totalCbm, totalTons) {
    if (totalCbm >= MIN_CBM && totalCbm <= MAX_CBM && totalTons <= MAX_TONS) {
      return { text: "Đạt", cls: "good", score: Math.abs(totalCbm - IDEAL_CBM) };
    }
  ...
  function renderEmpty(message) {
    ...
    bestInfoEl.textContent = "CBM: - | Tấn: -";
    resultCountEl.textContent = "0";
    resultTableEl.innerHTML = `<tr><td colspan="5" class="empty">${message}</td></tr>`;
  }

  function calculate() {
    const thickness = n(thicknessEl.value);
    const width = n(widthEl.value);
    const length = n(lengthEl.value);
    const density = n(densityEl.value);
    let pcsMin = i(pcsMinEl.value);
    let pcsMax = i(pcsMaxEl.value);
    ...
    for (let pcsPerBundle = pcsMin; pcsPerBundle <= pcsMax; pcsPerBundle++) {
      ...
      const bundles = Math.min(bundlesByMaxCbm, bundlesByMaxWeight);
    ...
    if (!rows.length) {
      ...
      bestInfoEl.textContent = "CBM: - | Tấn: -";
      resultCountEl.textContent = "0";
      resultTableEl.innerHTML = `<tr><td colspan="5" class="empty">Không có phương án phù hợp</td></tr>`;
      return;
    }

    rows.sort((a, b) => a.score - b.score || Math.abs(a.totalCbm - IDEAL_CBM) - Math.abs(b.totalCbm - IDEAL_CBM));

    const best = rows[0];
    bestPlanEl.textContent = `${best.pcsPerBundle} tấm/kiện · ${best.bundles} kiện`;
    bestInfoEl.textContent = `CBM: ${fmt(best.totalCbm, 3)} | Tấn: ${fmt(best.totalTons, 3)}`;
